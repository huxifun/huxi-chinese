...
fn main() {
    ...
    //let zj: String = fs::read_to_string("3500.txt").unwrap();
    ...
    // 合并
    let re: Vec<String> = lines.iter().map(|(k, v)| format!("{} {}", k, v)).collect();

    let mut buffer = fs::File::create("../fcitx-8000.txt").unwrap();
